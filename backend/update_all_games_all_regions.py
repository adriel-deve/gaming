"""
Update pages with ALL games and ALL regions comparison
Creates: data/all-games-all-regions.json with full price comparison
Regions: Americas (BR, US, CA, MX, AR, CL, CO, PE) + Europe (GB, DE, FR, ES, IT, PT, AU, ZA)
"""
import json
import os

print("="*60)
print("UPDATING ALL GAMES WITH ALL REGIONS COMPARISON")
print("="*60)

# All supported regions
ALL_REGIONS = ['BR', 'US', 'CA', 'MX', 'AR', 'CL', 'CO', 'PE', 'GB', 'DE', 'FR', 'ES', 'IT', 'PT', 'AU', 'ZA']
MAIN_REGIONS = ['BR', 'US', 'CA', 'MX', 'GB', 'DE', 'AU']  # For 4-region file
...
# Save all regions file
output_path_all = os.path.join('..', 'data', 'all-games-all-regions.json')
with open(output_path_all, 'w', encoding='utf-8') as f:
    json.dump(all_games_data, f, ensure_ascii=False, indent=None, separators=(',', ':'))

file_size = os.path.getsize(output_path_all) / (1024 * 1024)
print(f"\n[OK] Saved to: {output_path_all}")
print(f"[OK] File size: {file_size:.2f} MB")

# Also update the 4-regions file for backwards compatibility (BR, US, CA, MX)
...
# Show examples
print("\n" + "="*60)
print("EXAMPLES (Games with most regions):")
print("="*60)

shown = 0
for game in sorted(all_games_data, key=lambda x: -x['num_regions']):
    if game['num_regions'] >= 14:
        print(f"\n{game['title']} ({game['num_regions']} regions)")
        print(f"  CHEAPEST: {game['cheapest_region']} @ R$ {game['cheapest_price_brl']:.2f}")

        # Show top 5 cheapest
        sorted_prices = sorted(game['prices'].items(), key=lambda x: x[1]['brl'])
        for region, price in sorted_prices[:5]:
